...
constructor(public validation: ValidationService,
  ...
  ngOnInit(): void {
    ...
    this.dataService.GetRequest('https://api.propertydata.co.uk/stamp-duty-calculator?key='+environment.PropertyDataKey+'&value='+this.calcData.PropertyValue+'&country='+this.calcData.Country+'&mode='+this.calcData.PurchaseType).subscribe(
      (res: Response) => 
        {
        var obj = JSON.parse(JSON.stringify(res));
         if(obj.status=="success"){
           ...
           if(this.calcData.ForeignBuyer=="1"){
             this.stampDutyLandTax=(parseFloat(obj.transaction_tax_payable)+(parseFloat(this.calcData.PropertyValue)*0.02)).toString();
           }else{
            this.stampDutyLandTax=obj.transaction_tax_payable.toString();
           }
           this.effectiveRate=((parseFloat(this.stampDutyLandTax)/parseFloat(this.calcData.PropertyValue))*100).toFixed(2);
         }
        }
    );
  }
